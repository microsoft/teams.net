...
public static partial class AppInvokeActivityExtensions
{
    public static App OnSetting(this App app, Func<IContext<MessageExtensions.SettingActivity>, Task> handler)
    {
        app.Router.Register(new Route()
        {
            ...
            {
                await handler(context.ToActivityType<MessageExtensions.SettingActivity>()).ConfigureAwait(false);
                return null;
            },
    ...
    public static App OnSetting(this App app, Func<IContext<MessageExtensions.SettingActivity>, CancellationToken, Task> handler)
    {
        app.Router.Register(new Route()
        {
            Name = string.Join("/", [ActivityType.Invoke, Name.MessageExtensions.Setting]),
            ...
            Handler = async context =>
            {
                await handler(context.ToActivityType<MessageExtensions.SettingActivity>(), context.CancellationToken).ConfigureAwait(false);
                return null;
            },
            Selector = activity => activity is MessageExtensions.SettingActivity
        });
