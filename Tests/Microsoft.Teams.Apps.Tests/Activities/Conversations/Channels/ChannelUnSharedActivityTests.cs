...
public class ChannelUnSharedActivityTests
{
    private readonly App _app = new();
    private readonly IToken _token = Globals.Token;
    private readonly Controller _controller = new();

    public ChannelUnSharedActivityTests()
    {
    ...
    public async Task Should_CallHandler()
    {
        ...
        _app.OnChannelUnShared(context =>
        {
            calls++;
            Assert.True(context.Activity.Type.IsConversationUpdate);
            Assert.Equal(ConversationUpdateActivity.EventType.ChannelUnShared, context.Activity.ChannelData?.EventType);
        ...
        var res = await _app.Process<TestPlugin>(_token, new ConversationUpdateActivity()
        {
            ChannelData = new()
            {
                EventType = ConversationUpdateActivity.EventType.ChannelUnShared
            }
        });
    ...
    public class Controller
    {
        ...
        [Conversation.ChannelUnShared]
        public void OnChannelUnShared([Context] IContext.Next next)
        {
            Calls++;
            next();
        }
