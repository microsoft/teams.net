...
[Obsolete]
public class ChannelRestoredActivityTests
{
    private readonly App _app = new();
    private readonly IToken _token = Globals.Token;
    private readonly Controller _controller = new();

    public ChannelRestoredActivityTests()
    {
    ...
    public async Task Should_CallHandler()
    {
        ...
        _app.OnChannelRestored(context =>
        {
            calls++;
            Assert.True(context.Activity.Type.IsConversationUpdate);
            Assert.Equal(ConversationUpdateActivity.EventType.ChannelRestored, context.Activity.ChannelData?.EventType);
        ...
        var res = await _app.Process<TestPlugin>(_token, new ConversationUpdateActivity()
        {
            ChannelData = new()
            {
                EventType = ConversationUpdateActivity.EventType.ChannelRestored
            }
        });
    ...
    public class Controller
    {
        ...
        [Conversation.ChannelRestored]
        public void OnChannelRestored([Context] IContext.Next next)
        {
            Calls++;
            next();
        }
