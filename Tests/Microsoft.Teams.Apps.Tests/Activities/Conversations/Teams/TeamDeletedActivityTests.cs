...
public class TeamDeletedActivityTests
{
    private readonly App _app = new();
    private readonly IToken _token = Globals.Token;
    private readonly Controller _controller = new();

    public TeamDeletedActivityTests()
    {
    ...
    public async Task Should_CallHandler()
    {
        ...
        _app.OnTeamDeleted(context =>
        {
            calls++;
            Assert.True(context.Activity.Type.IsConversationUpdate);
            Assert.Equal(ConversationUpdateActivity.EventType.TeamDeleted, context.Activity.ChannelData?.EventType);
        ...
        var res = await _app.Process<TestPlugin>(_token, new ConversationUpdateActivity()
        {
            ChannelData = new()
            {
                EventType = ConversationUpdateActivity.EventType.TeamDeleted
            }
        });
    ...
    public class Controller
    {
        ...
        [Conversation.TeamDeleted]
        public void OnTeamDeleted([Context] IContext.Next next)
        {
            Calls++;
            next();
        }
