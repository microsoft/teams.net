...
public class TeamArchivedActivityTests
{
    ...
    public async Task Should_CallHandler()
    {
        ...
        _app.OnTeamArchived(context =>
        {
            calls++;
            Assert.True(context.Activity.Type.IsConversationUpdate);
            Assert.Equal(ConversationUpdateActivity.EventType.TeamArchived, context.Activity.ChannelData?.EventType);
    ...
    public class Controller
    {
        ...
        [Conversation.TeamArchived]
        public void OnTeamArchived([Context] IContext.Next next)
        {
            Calls++;
            next();
        }
