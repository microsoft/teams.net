...
A pre-authorized URL is short-lived, on the order of an hour, and the exact lifetime is set per URL rather than fixed. Do not depend on a particular number. Once it lapses `DownloadAsync()` raises `FileUrlExpiredException` and those bytes are unreachable: there is no recovery, and the file has to be sent again.
...
The handler code is the same: `ListAsync()` then `DownloadAsync()`, with the SDK selecting the agent's own credential rather than the app's. A failed Graph read raises `FileRetrievalException`, which names the identity that was refused and links the permissions documentation for it.
